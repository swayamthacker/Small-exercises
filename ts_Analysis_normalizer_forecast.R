...
#This code is essentially the same as the tsAnalysis.R code, except
#this code is useful for doing the same analysis on an individual ticker,
#as compared to the analysis on the entire universe of tickers
...
data_train = subset(data_train,index_normalizer != 0)
...
data_test = subset(data_train,index_normalizer != 0)

#unique list of tickers
uniqueTabs = unique(data_test$tab)

#added 
uniqueTabs = uniqueTabs[138] 
#change the above line based on the specific ticker you want to analyze
#end of added
...
for(tb in uniqueTabs) 
{
  ...
  data_trial = data.frame("date" = data_trial$date,
                          "index_normalizer" = data_trial$index_normalizer)
  
  #data_test_sub = subset(data_test,date > "2016-12-08")
  
  data_test_sub = subset(data_test,tab == tb)
  
  data_test_sub = data.frame("date" = data_test_sub$date,"index_normalizer" = 
                               data_test_sub$index_normalizer)
  ...
  gas <- ts(data_trial$index_normalizer,frequency = 365.25,
  ...
  accr <- data.frame(accuracy(fc, data_test_sub$index_normalizer))
