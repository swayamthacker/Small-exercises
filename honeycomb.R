...
fit.RegSpline_2 = lm(salesIndex~bs(TransactionIndex,df=2),data=data_trial_normalized) #adj R2 = 80.83
fit.RegSpline_3 = lm(salesIndex~bs(TransactionIndex,df=3),data=data_trial_normalized)#adj R2 = 80.83
fit.RegSpline_4 = lm(salesIndex~bs(TransactionIndex,df=4),data=data_trial_normalized)#adj R2 = 80.84
fit.RegSpline_5 = lm(salesIndex~bs(TransactionIndex,df=5),data=data_trial_normalized)#adj R2 = 80.87
...
print(subset(final.df,tab=="AMZN")) #value prediction with a sample ticker
...
tabData = data_test_sub
tabData.df <-  data.frame(tab=tabData$tab,TransactionIndex=tabData$TransactionIndex)
final.df <- data.frame(predict(fit.RegSpline_2,tabData.df,interval="predict",level=0.95))
final.df$salesIndex = tabData$salesIndex
final.df$TestValueInRange <- ifelse((final.df$salesIndex >= final.df$lwr) & (final.df$salesIndex <= final.df$upr),"YES","NO")
final.df$tab = tabData$tab
...
tabData = data_test_sub
tabData.df <-  data.frame(tab=tabData$tab,TransactionIndex=tabData$TransactionIndex)
final.df <- data.frame(predict(fit.RegSpline_3,tabData.df,interval="predict",level=0.95))
final.df$salesIndex = tabData$salesIndex
final.df$TestValueInRange <- ifelse((final.df$salesIndex >= final.df$lwr) & (final.df$salesIndex <= final.df$upr),"YES","NO")
final.df$tab = tabData$tab
...
tabData = data_test_sub
tabData.df <-  data.frame(tab=tabData$tab,TransactionIndex=tabData$TransactionIndex)
final.df <- data.frame(predict(fit.RegSpline_4,tabData.df,interval="predict",level=0.95))
final.df$salesIndex = tabData$salesIndex
final.df$TestValueInRange <- ifelse((final.df$salesIndex >= final.df$lwr) & (final.df$salesIndex <= final.df$upr),"YES","NO")
final.df$tab = tabData$tab
...
tabData = data_test_sub
tabData.df <-  data.frame(tab=tabData$tab,TransactionIndex=tabData$TransactionIndex)
final.df <- data.frame(predict(fit.RegSpline_5,tabData.df,interval="predict",level=0.95))
final.df$salesIndex = tabData$salesIndex
final.df$TestValueInRange <- ifelse((final.df$salesIndex >= final.df$lwr) & (final.df$salesIndex <= final.df$upr),"YES","NO")
final.df$tab = tabData$tab
...
print(subset(final.df,tab=="AMZN"))
...
fit.RegSpline_2 = lm(index_amount~bs(index_transaction,df=2),data=data_trial) #adj R2 = 81.79
fit.RegSpline_3 = lm(index_amount~bs(index_transaction,df=3),data=data_trial)#adj R2 = 80.83
fit.RegSpline_4 = lm(index_amount~bs(index_transaction,df=4),data=data_trial)#adj R2 = 80.84
fit.RegSpline_5 = lm(index_amount~bs(index_transaction,df=5),data=data_trial)#adj R2 = 80.87
...
#Without boxcox, poly regression
#tabData = subset(data_test_sub,tab == "ANF" | tab == "AMZN" | tab == "DNKN" | tab == "CMG" | tab == "MCD" | tab == "SBUX" | tab == "YUM-TB" )
tabData = data_test_sub
tabData.df <-  data.frame(tab=tabData$tab,index_transaction=tabData$index_transaction)
final.df <- data.frame(predict(fit.2,tabData.df,interval="predict",level=0.95))
...
#Compare the linear model to results from polynomial regression,regression splines, smoothing splines, 
#natural splines, ridge regression. Argue that it makes sense to keep the model linear
