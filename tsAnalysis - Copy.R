...
{
  data_trial = subset(data_train,tab==tb & date <= "2015-12-31")
  ...
  data_test_sub = subset(data_test,tab == tb & date >= "2016-01-01")
  ...
  for(i in 1:25)
  {
    fit <- auto.arima(gas, xreg=fourier(gas, K=i), seasonal=TRUE)
  ...
  #plot(fc)
  #xr_1 = c(start(gas),end(gas)+1)
  #wasnt there ts_xr = ts(xr,start = start(gas),end=end(gas),frequency = frequency(gas))
  #plot(gas, xlim=xr_1, ylab = "Sales", xlab = "Days")
  #lines(fc$mean,x=seq(end(gas),end(gas)+1,length=363), col="blue")
  #lines(fc$lower[,2],x=seq(end(gas),end(gas)+1,length=363), col="green")
  #lines(fc$upper[,2],x=seq(end(gas),end(gas)+1,length=363), col="red")
...
TickersAndForecast$forecast <- ForecastOfTicker
...
#Now i have a table of Tickers and their relevant RMSE for the
#forecast for 2016
#Sort that table and for the top 10 tickers, get their forecast
#for 2016 from the tickersAndForecast table. From that forecast we can 
#compute the YOY change,estimate sales value for next quarter
...
lines(Data2016$R, x=seq(end(gas),end(gas)+1,length=365), col="blue")
lines(Data2016$O, x=seq(end(gas),end(gas)+1,length=365), col="green")
lines(Data2016$P, x=seq(end(gas),end(gas)+1,length=365), col="red")
accr_bk = accuracy(Data2016$R,data_test_sub$index_amount)
